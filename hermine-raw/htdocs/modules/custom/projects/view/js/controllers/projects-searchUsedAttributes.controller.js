...
(function() {
	angular.module("hermine").controller("ProjectsSearchUsedAttributesController", ["$scope", "UsersFactory", "ProjectFactory", "$http", "$q", "$routeParams", "$location", "ngDialog", "$filter", function($scope, UsersFactory, ProjectFactory, $http, $q, $routeParams, $location, ngDialog, $filter) {

		
        $scope.getItemsWithUsedAttribute = [];
        BRUNCH.showSpinner();
        ProjectFactory.getItemsWithUsedAttribute($scope.currentProject.refKey, $scope.searchUsedAttribute.name, $scope.searchUsedAttribute.attribute, true).then(function(data) {
            $scope.getItemsWithUsedAttribute = data;
            BRUNCH.hideSpinner();
        });
