...
class AttributesAdapter extends AbstractAdapterBase {
	...
	private $query_sp_list_all_storage_for_project			= "CALL sp_list_all_storage_for_project('{ref}')";
	...
	private $query_sp_delete_storage 						= "CALL sp_delete_storage('{referenceId}');";
	...
	function getAllStoragesForProject($id)
	{
		if ($id != "")
		{
			$query = str_replace("{ref}", $id, $this->query_sp_list_all_storage_for_project);
	...
	function saveStorage($attributeTypeId,$attributeValues,$referenceId,$groupId) {
	...
	function deleteStorage($referenceId) {
		
		// delete
		$delQuery = str_replace("{referenceId}", $referenceId, $this->query_sp_delete_storage);
